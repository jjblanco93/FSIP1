...
def tree_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            printf("Expandidos: %d", fringe.expandido)
            return node
...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
            printf("Expandidos: %d, Recorridos: ", fringe.expandido)
            return node
...
def Lista_coste(problem):
    """Search the cheapest movement from current node to the next node."""
    return graph_search(problem, ListaCoste(problem)) # SortedListByCost -> fringe
